...
int main(int argc, char *argv[])
{
    ...
    QSystemTrayIcon trayIcon(QIcon("icon.jpg"));
    trayIcon.show();

    // Create a menu for the system tray icon
    QMenu trayMenu;
    QAction action("Show Notification", &a);
    trayMenu.addAction(&action);
    trayIcon.setContextMenu(&trayMenu);

    // Connect the action to show a notification
    QObject::connect(&action, &QAction::triggered, [&](){
        trayIcon.showMessage("Notification Title", "Notification Message", QSystemTrayIcon::Information, 50000);
    });
    ...
    QObject::connect(&timer, &QTimer::timeout, [&](){
        static int count = 0;
        label.setText(QString("Timer: %1").arg(++count));
    });
